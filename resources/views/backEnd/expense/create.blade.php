            ...
            <form action="{{ route('expenses.store') }}" method="POST" class="row">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select class="form-control" name="expense_cat_id">
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" @selected((string) old('expense_cat_id') === (string) $category->id)>{{ $category->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="col-md-3">
                    <div class="form-group mb-3">
                        ...
                        <input type="date" name="transaction_date" class="form-control" value="{{ old('transaction_date', now()->format('Y-m-d')) }}">
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group mb-3">
                        ...
                        <select class="form-control" name="financial_account_id">
                            ...
                            @foreach($financialAccounts as $financialAccount)
                                <option value="{{ $financialAccount->id }}" @selected((string) old('financial_account_id') === (string) $financialAccount->id)>{{ $financialAccount->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select class="form-control" name="account_head_id">
                            ...
                            @foreach($accountHeads as $accountHead)
                                <option value="{{ $accountHead->id }}" @selected((string) old('account_head_id') === (string) $accountHead->id)>{{ $accountHead->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
